/**
 * AI 提示词预设（仿 FleurPDF 的体系，为 FleurAnnotation 扩展）
 *
 * 设计要点：
 * - 预设只定义「角色定位」（body），不含字数约束。
 * - 字数约束由调用场景决定，动态追加：
 *   · 侧边栏 AI 批注 → 需要精炼，基准字数默认 250（可在设置里调整；原文过长时放宽）
 *   · 正文「询问 AI」→ 对话场景，不设字数限制
 ...
 * @param customPrompts 自定义模式下的用户文本：三个自定义槽的数组，或单个字符串（兼容旧逻辑）
